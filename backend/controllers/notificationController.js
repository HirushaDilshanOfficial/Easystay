...
// @desc    Get notifications for an owner (by string ID)
// @route   GET /api/notifications/owner/:ownerId
// @access  Public (Simplified for now)
exports.getOwnerNotifications = async (req, res, next) => {
    try {
        const notifications = await Notification.find({ user: req.params.ownerId })
            .sort({ createdAt: -1 })
            .limit(20);
        res.status(200).json({ success: true, data: notifications });
    } catch (err) {
