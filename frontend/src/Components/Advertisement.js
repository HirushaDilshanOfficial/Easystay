...
export default function Advertisement() {
  ...
  const handleFileRead = (e, field) => {
    ...
    reader.onloadend = () => {
      ...
      if (field === "imageUrl") {
        ...
        setSlipPreview(val);
        setForm(f => ({ ...f, paymentSlip: val }));
        setErrors(e => ({ ...e, paymentSlip: null }));
      }
    };
  ...
  const handleSubmit = async () => {
    ...
    if (!slipPreview) {
      ...
      showToast("Payment slip is required", "error");
      return;
    }
    setSubmitting(true);
    try {
  ...
  const handleEdit = (ad) => {
    ...
    setForm({
      name: ad.name, description: ad.description, phoneNumber: ad.phoneNumber,
      price: ad.price, imageUrl: ad.imageUrl || "", paymentSlip: ad.paymentSlip || "",
      packageType: pkg.id,
    });
  ...
  return (
    <div style={{ fontFamily: "'Figtree', sans-serif", minHeight: "100vh", background: "#F0F4FF", color: "#1E293B", position: "relative" }}>
      ...
      <div style={{ position:"fixed", inset:0, zIndex:0, pointerEvents:"none",
        background:"radial-gradient(ellipse 80% 50% at 50% -5%, rgba(37,99,235,0.08) 0%, transparent 65%), radial-gradient(ellipse 50% 40% at 85% 85%, rgba(37,99,235,0.04) 0%, transparent 55%)" }} />
        ...
        {view === "dashboard" && (
          <div style={{ animation:"fadeUp 0.4s ease" }}>
            <div style={{ marginBottom:32 }}>
              ...
              <p style={{ fontSize:14, color:"#64748B" }}>Create and manage your Advertisements</p>
            </div>

            {/* Stats */}
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(190px,1fr))", gap:14, marginBottom:32 }}>
              {[
                { label:"Total Ads", val:advertisements.length, color:"#2563EB", bg:"#EFF6FF" },
                ...
                <div key={s.label} style={{
                  ...
                  <div style={{ marginBottom:14 }}>
                    <div style={{ fontSize:10, color: s.color, fontWeight:800, textTransform:"uppercase", letterSpacing:"0.1em" }}>
                      {s.label}
                    </div>
                  </div>
                  <div style={{ fontSize:38, fontWeight:900, color:"#1E293B", letterSpacing:"-2px", lineHeight:1 }}>{s.val}</div>
                  <div style={{ marginTop:10, fontSize:11, color: s.color, fontWeight:600 }}>
                ...
                advertisements.map((ad, i) => {
                  ...
                  return (
                    <div key={ad._id} className="tr" style={{ display:"grid", gridTemplateColumns:"2fr 3fr 1.3fr 1.2fr 1.2fr 100px",
                      padding:"15px 24px", borderBottom: i < advertisements.length-1 ? "1px solid rgba(37,99,235,0.07)" : "none",
                      ...
                      <div style={{ display:"flex", gap:8 }}>
                        {[[<EditIcon/>, "#3B82F6", () => handleEdit(ad), "Edit"],
                          ...
                          <button key={title} className="ib" title={title} onClick={action}
                            style={{ width:32, height:32, borderRadius:8, border:"none", background:`${color}12`, color:color,
                  ...
                  {stepLabels.map((label, i) => (
                    <div key={i} style={{ display:"flex", flexDirection:"column", alignItems:"center", flex: i<2 ? 1 : 0 }}>
                      ...
                      <div style={{ fontSize:11, fontWeight:600, marginTop:7, whiteSpace:"nowrap",
                        color: step === i+1 ? "#2563EB" : "#94A3B8" }}>{label}</div>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div style={{ background:"#FFFFFF", border:"1px solid rgba(37,99,235,0.12)", borderRadius:20, padding:32, animation:"slideIn 0.3s ease" }}>
                  <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:20 }}>
                    <span style={{ display:"inline-flex", alignItems:"center", padding:"4px 12px", borderRadius:20,
                      fontSize:12, fontWeight:700, background:`${selectedPackage?.color}18`, color:selectedPackage?.color,
                      border:`1px solid ${selectedPackage?.color}35` }}>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:18 }}>
                    ...
                    <div className={touched.name && errors.name ? "field-error" : ""}>
                      <Field label="Advertisement Name" error={errors.name} touched={touched.name} required>
                        <input type="text" placeholder="e.g. Sunrise Boarding House"
                          value={form.name}
                          onChange={e => handleChange("name", e.target.value)}
                          onBlur={() => handleBlur("name")}
                          style={{ width:"100%", padding:"12px 15px", borderRadius:10, background:"#FFFFFF",
                            border:`1px solid ${borderColor("name")}`,
                            boxShadow: inputGlow("name"),
                            color:"#1E293B", fontSize:14, transition:"all 0.2s" }} />
                      </Field>
                    </div>

                    {/* Phone */}
                    <div className={touched.phoneNumber && errors.phoneNumber ? "field-error" : ""}>
                      <Field label="Phone Number" error={errors.phoneNumber} touched={touched.phoneNumber} required
                        hint="Sri Lankan numbers only (e.g. 0771234567)">
                        <input type="text" placeholder="e.g. 0771234567"
                          value={form.phoneNumber}
                          onChange={e => handleChange("phoneNumber", e.target.value)}
                          onBlur={() => handleBlur("phoneNumber")}
                          style={{ width:"100%", padding:"12px 15px", borderRadius:10, background:"#FFFFFF",
                            border:`1px solid ${borderColor("phoneNumber")}`,
                            boxShadow: inputGlow("phoneNumber"),
                            color:"#1E293B", fontSize:14, transition:"all 0.2s" }} />
                      </Field>
                    </div>

                    {/* Price — locked to package */}
                    <div>
                      <label style={{ display:"block", fontSize:12, fontWeight:600, color:"#64748B", marginBottom:7, textTransform:"uppercase", letterSpacing:"0.09em" }}>
                        Package Price
                      </label>
                      <div style={{ position:"relative" }}>
                        ...
                        <input type="text" readOnly value={Number(form.price).toLocaleString()}
                          style={{ width:"100%", padding:"12px 15px 12px 50px", borderRadius:10,
                        ...
                        <span style={{ position:"absolute", right:12, top:"50%", transform:"translateY(-50%)",
                          ...
                          background:`${selectedPackage?.color}18`, color:selectedPackage?.color,
                          border:`1px solid ${selectedPackage?.color}30` }}>
                    ...
                    <div style={{ gridColumn:"1/-1" }} className={touched.description && errors.description ? "field-error" : ""}>
                      <Field label="Description" error={errors.description} touched={touched.description} required>
                        <textarea
                          placeholder="Describe your boarding house — location, rooms, amenities, rules, nearby facilities..."
                          value={form.description}
                          onChange={e => handleChange("description", e.target.value)}
                          onBlur={() => handleBlur("description")}
                          style={{ width:"100%", padding:"12px 15px", borderRadius:10, minHeight:100,
                            background:"#FFFFFF",
                            border:`1px solid ${borderColor("description")}`,
                            boxShadow: inputGlow("description"),
                    ...
                    <div style={{ gridColumn:"1/-1" }}>
                      <label style={{ display:"block", fontSize:10, fontWeight:700, color:"#94A3B8", marginBottom:8, textTransform:"uppercase", letterSpacing:"0.1em" }}>
                        Advertisement Photo <span style={{ color:"#94A3B8", fontWeight:400, textTransform:"none", letterSpacing:0 }}>(optional)</span>
                      </label>
                      <div onClick={() => imageRef.current.click()}
                        style={{ border: imagePreview ? "2px solid rgba(59,130,246,0.4)" : "2px dashed #BFDBFE",
                          borderRadius:12, padding:"24px 20px", textAlign:"center", cursor:"pointer", transition:"all 0.2s",
                          background: imagePreview ? "rgba(37,99,235,0.04)" : "#FAFCFF" }}>
