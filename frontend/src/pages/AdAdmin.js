...
// ─── Overlay Component 
export default function AdAdmin({ hideSidebar = false }) {
  ...
  const approve = async (ad) => {
    setBusy(ad._id + "_ok");
    try {
      const r = await api.put(`/advertisements/${ad._id}/approve`, { adminNote: "" });
      showT(`"${ad.name}" approved`); load(); setViewAd(null);
  ...
  const reject = async () => {
    ...
    try {
      const r = await api.put(`/advertisements/${rejectAd._id}/reject`, { adminNote: note });
      showT(`"${rejectAd.name}" rejected`); load(); setRejectAd(null); setNote(""); setViewAd(null);
    } catch { showT("Server error", "error"); }
    setBusy(null);
  };
  ...
  const deleteAd_confirm = async () => {
    ...
    try {
      await api.delete(`/advertisements/${deleteAd._id}`);
